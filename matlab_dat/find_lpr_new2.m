function find_lpr_new2(file,isy,relu,area,metal,ovp_select,ncol)
...
d=d(1:end-ncol,:);
d=flipud(d);
pp=spline(d(:,2),d(:,1));%x=d(:,2) potential, y=d(:,1)  current
[breaks,coefs,l,k,dim]=unmkpp(pp);

for k=1:length(coefs)
coefs1(k,:)=polyder(coefs(k,:));
end

pp=mkpp(breaks,coefs1);
v=ppval(pp,d(:,2));
degg=121;
v=sgolayfilt(v,1,degg);%The filter should be used in this position
d(:,1)=sgolayfilt(d(:,1),1,degg);
v=v(find(v>0));
dvx=d(find(v>0),1);
v=1./v;
figure(4)
...
g=plot(d(:,2),d(:,1),'--',d(row,2),fitres,'-r');
...
plot(ovp,log(abs(cud)),ovp_s,fitres,'-r','Linewidth',2);
...
g2=plot(sec_data(1:end,1),sec_data(1:end,2),'--',sec_Range(:,1),sec_Range(:,2),'-r');
xlim([sec_data(end,1) 0])
set(g2(1),'linewidth',2);
set(g2(2),'linewidth',2.5);
xlabel('i (mA)');
ylabel('R (\Omega)');
...
g2=plot(sec_data(1:end,1),sec_data(1:end,2)/(area));
xlim([sec_data(end,1) 0]);
set(g2(1),'linewidth',2);
xlabel('j (mA/cm^2)');
ylabel('\rho  (\Omega/cm^2)' );
title(['LPR',  '=' , num2str(LPR_cf/area),' ','\Omega/cm^2','  ','@',' ', num2str(isy/area),' ','mA/cm^2']);
